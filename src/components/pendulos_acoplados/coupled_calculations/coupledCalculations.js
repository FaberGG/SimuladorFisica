...
function calculateAmplitudeRelation(inertia, inertia2, K1, K2, Kc, omega, omega2) {
    // Relación de amplitud para el primer modo normal: A1 / B1
    const A1_B1 = Kc / (K1 + Kc - inertia * omega);
  
    // Relación de amplitud para el segundo modo normal: A2 / B2
    const A2_B2 = Kc / (K2 + Kc - inertia2 * omega2);
...
export function position1(time, A1, A2, omega, omega2, phi) {
    return (
      A1 * Math.cos(omega * time + phi) +
      A2 * Math.cos(omega2 * time + phi)
    );
  }
